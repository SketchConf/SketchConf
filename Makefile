# Flags
CXXFLAGS := -Wall -Werror -Wno-unused-function -Wno-unused-variable -Wno-unused-private-field -Wno-register -Wno-c++11-narrowing -std=c++20
FFLAGS :=
BFLAGS := -d -v
LDFLAGS :=
LOGMODE :=
...
TRIVIAL ?= 0
ifeq ($(TRIVIAL), 0)
TRIVIALFLAG = __NONE__
else
TRIVIALFLAG = TRIVIAL
endif


CXXFLAGS += -D $(TRUNCFLAG) -D $(REUSEFLAG) -D $(TRIVIALFLAG)
DEBUG_CXXFLAGS += -D DEBUGMODE -D $(TRUNCFLAG) -D $(REUSEFLAG) -D $(TRIVIALFLAG)
# Compilers
CXX := clang++
FLEX := flex
BISON := bison
...
# DEBUG_OBJECTS := $(patsubst $(SRC_DIR)/%.cpp, $(DEBUG_DIR)/%.cpp.o, $(SRC))
# DEBUG_OBJECTS := $(patsubst $(DEBUG_DIR)/%.cpp, $(DEBUG_DIR)/%.cpp.o, $(DEBUG_OBJECTS))
...
# all:
# 	echo $(CPPFILES) $(LEXFILES) $(YACCFILES)
